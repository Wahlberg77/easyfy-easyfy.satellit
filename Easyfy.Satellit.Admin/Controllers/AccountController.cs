...
namespace Easyfy.Satellit.Admin.Controllers
{
  ...
  public class AccountController : RavenController
  {
    ...
    public async Task<ActionResult> Register(RegisterViewModel model)
    {
      if (ModelState.IsValid)
      {
        ...
        if (result.Succeeded)
        {
          ...
          string callbackUrl = await SendEmailConfirmationTokenAsync(user.Id, "template-confirm-email");
    ...
    private async Task<string> SendEmailConfirmationTokenAsync(string userID, string template)
    {

      string code = await _userManager.GenerateEmailConfirmationTokenAsync(userID);
      var callbackUrl = Url.Action("ConfirmEmail", "Account",
         new { userId = userID, code = code }, protocol: Request.Url.Scheme);
      await _userManager.SendEmailAsync(userID, template, callbackUrl);
